...
class Ticket():
    ...
    def check_validity(self):
        ...
        for f in self.fields:
            ...
            for rule in rules:
                if f in range(rule[0][0], rule[0][1]+1) or f in range(rule[1][0], rule[1][1]+1):
                    valid = True
                    break
            if not valid:
                invalid_sum += f
    ...
    def field_in_rule(self, position, rule):
        if rule[0][0] <= self.fields[position] <= rule[0][1] or rule[1][0] <= self.fields[position] <= rule[1][1]:
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    your_ticket =  [int(x) for x in file.split('\n\n')[1].strip().split('\n')[1].split(',')]
    ticket_value = 1
